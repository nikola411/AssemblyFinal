...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, eAssemblyIdentifier&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const eAssemblyIdentifier& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    ...
    parser (Driver& drv_yyarg, AssemblyAdapter& assembly_yyarg);
    virtual ~parser ();
